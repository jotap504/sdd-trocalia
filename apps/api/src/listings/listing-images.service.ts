...
export class ListingImagesService {
  ...
  private async assertOwner(userId: string, listingId: string) {
    const [listing] = await this.db
      .select({ id: schema.listings.id, userId: schema.listings.userId, type: schema.listings.type })
  ...
  private async assertBelowMaxImages(listingId: string, type: string) {
    ...
    const max = type === 'premium' ? LISTING.MAX_IMAGES_PREMIUM : LISTING.MAX_IMAGES_STANDARD
    if (current >= max) throw new BadRequestException(`MAX_IMAGES_REACHED (max: ${max})`)
  }
